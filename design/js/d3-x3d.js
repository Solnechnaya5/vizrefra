...
(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined"
    ...
    ? define(["d3", "d3-shape", "d3-array", "d3-interpolate"], factory)
    : ((global =
        typeof globalThis !== "undefined" ? globalThis : global || self),
      ((global.d3 = global.d3 || {}),
      (global.d3.x3d = factory(global.d3, global.d3, global.d3, global.d3))));
  ...
  function fromCurve(values, curveFunction) {
    var epsilon =
      arguments.length > 2 && arguments[2] !== undefined
        ...
        : 0.00001;
    var samples =
      arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 100;
    // eslint-disable-line max-params
  ...
  function dataTransform(data) {
    ...
    var multiColumnTotals = function multiColumnTotals(data) {
      return data.reduce(function (totals, row) {
        row.values.forEach(function (d) {
          ...
          totals[columnName] =
            typeof totals[columnName] === "undefined" ? 0 : totals[columnName];
      ...
      return data
        ...
        .reduce(function (minimums, row) {
          coordinates.forEach(function (coord) {
            minimums[coord] =
              coord in minimums
                ? d3__namespace.min([minimums[coord], +row[coord]])
                : row[coord];
          });
          return minimums;
        }, {});
      ...
      return data
        ...
        .reduce(function (maximums, row) {
          coordinates.forEach(function (coord) {
            maximums[coord] =
              coord in maximums
                ? d3__namespace.max([maximums[coord], +row[coord]])
                : row[coord];
          });
          return maximums;
        }, {});
    ...
    var multiCoordinatesExtent = function multiCoordinatesExtent(data) {
      return coordinates.reduce(function (extents, coord) {
        extents[coord] = [
          multiCoordinatesMin(data)[coord],
          multiCoordinatesMax(data)[coord],
        ];
        return extents;
      }, {});
    };
  ...
  function colorParse(color) {
    ...
    var x3dMatch =
      /^(0+\.?\d*|1\.?0*)\s+(0+\.?\d*|1\.?0*)\s+(0+\.?\d*|1\.?0*)$/.exec(color);
    ...
    var rgbMatch = /^rgb\((\d{1,3}),\s{0,1}(\d{1,3}),\s{0,1}(\d{1,3})\)$/.exec(
      color
    );
    ...
    var my = function my(selection) {
      selection.each(function (data) {
        ...
        var areaData = function areaData(data) {
          ...
          var coords = values
            .map(function (pointThis, indexThis, array) {
        ...
        var shape = function shape(el) {
          ...
          shape.html(function (d) {
            return '\n\t\t\t\t\t<IndexedFaceset coordIndex="'
              ...
              .concat(colorParse(color), '" transparency="')
              .concat(
                transparency,
                '"></Material>\n\t\t\t\t\t</Appearance>\n\t\t\t\t'
              );
        ...
        var area = element.selectAll(".area").data(
          function (d) {
            return areaData(d);
          },
          function (d) {
  ...
  function componentAreaMultiSeries() {
    ...
    var my = function my(selection) {
      selection.each(function (data) {
        ...
        var areaGroup = element.selectAll(".areaGroup").data(
          function (d) {
            return d;
          },
          function (d) {
    ...
    var my = function my(selection) {
      selection.each(function () {
        ...
        tickValues = scale.ticks
          ...
          : scale.domain();
        var tickFormatDefault = scale.tickFormat
        ...
        if (tickFormat !== "") {
          ...
          labels
            ...
            .attr(
              "translation",
              tickDirectionVector.map(function (d, i) {
                return (
                  labelInset * d * tickPadding +
                  ((labelInset + 1) / 2) * tickSize * tickDirectionVector[i]
                );
              })
            )
            .append("Billboard")
    ...
    var my = function my(selection) {
      selection.each(function () {
        ...
        zxAxis
          ...
          .tickSize(xScale.range()[1] - xScale.range()[0])
          .color("black")
          .labelPosition(labelPosition); // We only want 2 sets of labels on the y axis if they are in distal position.
  ...
  var dispatch = d3__namespace.dispatch(
    "d3X3dClick",
    "d3X3dMouseOver",
    "d3X3dMouseOut"
  );
  ...
  function attachEventListners(el) {
    el.attr("onclick", "d3.x3d.events.forwardEvent(event);").on(
      "click",
      function (e) {
        dispatch.call("d3X3dClick", this, e);
      }
    );
    el.attr("onmouseover", "d3.x3d.events.forwardEvent(event);").on(
      "mouseover",
      function (e) {
        dispatch.call("d3X3dMouseOver", this, e);
      }
    );
    el.attr("onmouseout", "d3.x3d.events.forwardEvent(event);").on(
      "mouseout",
      function (e) {
  ...
  function getEventPagePoint(event) {
    ...
    if ("getBoundingClientRect" in document.documentElement) {
      ...
      pageX = Math.round(
        event.layerX + (box.left + paddingLeft + borderLeftWidth + scrolLeft)
      );
      pageY = Math.round(
        event.layerY + (box.top + paddingTop + borderTopWidth + scrollTop)
      );
      ...
      x3dom.debug.logError(
        "Unable to find getBoundingClientRect or webkitConvertPointFromPageToNode"
      );
    }
  ...
  function componentBars() {
    ...
    var my = function my(selection) {
      selection.each(function (data) {
        ...
        var bars = element.selectAll(".bar").data(
          function (d) {
            return d.values;
          },
          function (d) {
  ...
  function componentBarsMultiSeries() {
    ...
    var my = function my(selection) {
      selection.each(function (data) {
        ...
        var barGroup = element.selectAll(".barGroup").data(
          function (d) {
            return d;
          },
          function (d) {
  ...
  function componentBubbles() {
    ...
    var my = function my(selection) {
      selection.each(function (data) {
        ...
        var bubbles = element.selectAll(".bubble").data(
          function (d) {
            return d.values;
          },
          function (d) {
  ...
  function componentBubblesMultiSeries() {
    ...
    var bubbles = componentBubbles()
      ...
      .colors(d3__namespace.schemeRdYlGn[8])
      .sizeRange([2, 2]);
    /**
     * Unique Array
     *
  ...
  function componentCrosshair() {
    ...
    var my = function my(selection) {
      selection.each(function (data) {
        ...
        var colorScale = d3__namespace
          .scaleOrdinal()
          .domain(Object.keys(dimensions))
          .range(colors); // Origin Ball
  ...
  function componentLight() {
    ...
    var my = function my(selection) {
      selection.each(function () {
        var light = d3__namespace
          .select(this)
          .selectAll("DirectionalLight")
          .data([null]);
        light
          .enter()
  ...
  function componentParticles() {
    ...
    var my = function my(selection) {
      selection.each(function (data) {
        ...
        var particles = element.selectAll(".particle").data(
          function (d) {
  ...
  function componentRibbon() {
    ...
    var my = function my(selection) {
      selection.each(function (data) {
        ...
        var ribbon = element.selectAll(".ribbon").data(
          function (d) {
            return ribbonData(d);
          },
          function (d) {
  ...
  function componentRibbonMultiSeries() {
    ...
    var my = function my(selection) {
      selection.each(function (data) {
        ...
        var ribbonGroup = element.selectAll(".ribbonGroup").data(
          function (d) {
            return d;
          },
          function (d) {
  ...
  function componentSurface() {
    ...
    var init = function init(data) {
      ...
      if (typeof colorScale === "undefined") {
        colorScale = d3__namespace
          ...
          .domain(valueExtent)
          .range(colors)
          .interpolate(d3__namespace.interpolateLab);
      }
    };
    /**
     * Constructor
    ...
    var my = function my(selection) {
      selection.each(function (data) {
        ...
        var surface = element.selectAll(".surface").data(
          function (d) {
  ...
  function componentVectorFields() {
    ...
    var init = function init(data) {
      ...
      var _dimensions = dimensions,
        ...
        dimensionY = _dimensions.y,
        dimensionZ = _dimensions.z;
      var extent = d3__namespace.extent(
        data.values.map(function (f) {
          var vx, vy, vz;
        ...
        var vectorData = function vectorData(d) {
          return d.values
            .map(function (f) {
              ...
              f.translation =
                xScale(f.x) + " " + yScale(f.y) + " " + zScale(f.z); // Calculate vector length

              f.value = vecLen; // Calculate transform-rotation attr

              f.rotation = []
                .concat(_toConsumableArray(vecRotate), [angleRotate])
                .join(" ");
              return f;
            })
            .filter(function (f) {
    ...
    my.quickView = function (view) {
      switch (view) {
        ...
        case "top":
          ...
          viewOrientation = [-0.86241, 0.3749, 0.34013, 1.60141];
          fieldOfView = 1.0;
          break;
  ...
  function createScene2(x3d, layers, classed) {
    ...
    scene
      ...
      .attr("skyColor", "1 1 1"); // Add layer groups
    ...
    var my = function my(selection) {
      ...
      selection.each(function (data) {
        ...
        viewpoint
          ...
          .viewPosition([77.63865, 54.6947, 104.38314]);
