...
<form  class="registration-form"  method="POST" action="<?php echo esc_url( rcp_get_current_url() ); ?>">

	<?php if( ! is_user_logged_in() ) { ?>
		<p class="registration-block_top"><?php printf( __( 'Already have an account? <a href="%s">Sign in</a>', 'rcp' ), esc_url( rcp_get_login_url( rcp_get_current_url() ) ) ); ?></p>

	<?php do_action( 'rcp_before_register_form_fields' ); ?>
		<div  class="registration_user-name">
		<input name="rcp_user_first" id="rcp_user_first" type="text" placeholder="Name" <?php if( isset( $_POST['rcp_user_first'] ) ) { echo 'value="' . esc_attr( $_POST['rcp_user_first'] ) . '"'; } ?>/>
        <span>*</span>
        </div>
	...
	if( $levels ) : ?>
		<?php if ( count( $levels ) > 1 ) : ?>
			<div class="registration-plan">
				<?php echo apply_filters ( 'rcp_registration_choose_subscription', __( '<h6 class="registration-plan_title">Membership Plan *</h6>', 'rcp' ) ); ?></в>
		<?php endif; ?>
		<div id="rcp_subscription_levels" class="registration-plan_conteiner">
	
			<?php foreach( $levels as $key => $level ) : ?>
				<?php if( rcp_show_subscription_level( $level->get_id() ) ) : ?>
				<div class="registration-plan_column
				<?php echo esc_attr( $level->get_id() ); ?>">
...
<label for="rcp_subscription_level_<?php echo esc_attr( $level->get_id() ); ?>">
						...
						<span class="rcp_level_duration">
							<?php echo ! $level->is_lifetime() ? $level->get_duration() . '&nbsp;' . rcp_filter_duration_unit( $level->get_duration_unit(), $level->get_duration() ) : __( 'unlimited', 'rcp' ); ?></span>
						<?php if ( $level->get_maximum_renewals() > 0 ) : ?> -->
							<span class="rcp_separator">&nbsp;-&nbsp;</span>
							<span class="rcp_level_bill_times">
								<?php printf( __( '%d total payments', 'rcp' ), 
								$level->get_maximum_renewals() + 1 ); ?></span>
					 <?php endif; ?>
						<div class="rcp_level_description"> 
							<?php echo $level->get_description(); ?></div>
					</label>
	...
	<fieldset class="rcp_discounts_fieldset">
		<p id="rcp_discount_code_wrap">
			<label for="rcp_discount_code">
				...
				<span class="rcp_discount_valid" style="display: none;"> - 
				<?php _e( 'Valid', 'rcp' ); ?></span>
