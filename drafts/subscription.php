...
?>
<div class="account-membership_top">
<h4><?php _e( 'Current', 'rcp' ); ?></h4>
<div class="account-membership_top-btns">

<?php
$links = array();

if ( $membership->can_update_billing_card() ) {
$links[] = '<div class="payment_change-plan acc-btn ">
<a href="' . esc_url( add_query_arg( 'membership_id', urlencode( $membership->get_id() ), get_permalink( $rcp_options['update_card'] ) ) ) . '" title="' . esc_attr__( 'Change Plan', 'rcp' ) . '" class="rcp_sub_details_update_card">' . __( 'Change Plan', 'rcp' ) . '</a>
</div>';
}

if ( $membership->can_renew() ) {
$links[] = apply_filters( 'rcp_subscription_details_action_renew', 
'<div class="payment_change-plan acc-btn ">
<a href="' . esc_url( rcp_get_membership_renewal_url( $membership->get_id() ) ) . 
'" title="' . esc_attr__( 'Renew your membership', 'rcp' ) . 
'" class="rcp_sub_details_renew">' . __( 'Renew your membership', 'rcp' ) . '</a></div>', 
$user_ID );
}

if ( $membership->upgrade_possible() ) {
$links[] = apply_filters( 'rcp_subscription_details_action_upgrade', 
'<div class="payment_change-plan acc-btn "><a href="' . esc_url( rcp_get_membership_upgrade_url( $membership->get_id() ) ) . 
'" title="' . esc_attr__( 'Upgrade membership', 'rcp' ) . '" class="rcp_sub_details_change_membership">' 
. __( 'Upgrade membership', 'rcp' ) . '</a></div>', $user_ID );
}
...
'<div class="payment_change-plan acc-btn ">
<a href="' . esc_url( rcp_get_membership_cancel_url( $membership->get_id() ) ) .
 '" title="' . esc_attr__( 'Cancel membership', 'rcp' ) . 
 '" class="rcp_sub_details_cancel" id="rcp_cancel_membership_' . 
 esc_attr( $membership->get_id() ) . '">' . __( 'Cancel membership', 'rcp' ) . '</a></div>', $user_ID );
}
...
?>
	...
	<div class="account_actual-status" id="rcp-account-overview">
		<header>
		<div class="account_actual-status_column1 ">
			<div class="account_actual-status_item"><?php _e( 'Membership', 'rcp' ); ?></div>
			<div class="account_actual-status_item"><?php _e( 'Status', 'rcp' ); ?></div>
			<div class="account_actual-status_item"><?php _e( 'Expiration / Renewal Date', 'rcp' ); ?></div>
			<?php if ( $has_payment_plan ) : ?>
				<div class="account_actual-status_item"><?php _e( 'Times Billed', 'rcp' ); ?></span>
			<?php endif; ?>
		...
		<?php if ( ! empty( $memberships ) ) : ?>
			<?php foreach ( $memberships as $membership ) : ?>
				<div class="account_actual-status_column2">
					<div class="account_actual-status_item account-status_mark-stop" data-th="<?php esc_attr_e( 'Membership', 'rcp' ); ?>">
						<?php echo esc_html( $membership->get_membership_level_name() ); ?>
			        </div>
					<div class="account_actual-status_item" data-th="<?php esc_attr_e( 'Status', 'rcp' ); ?>">
						<?php rcp_print_membership_status( $membership->get_id() ); ?>
					</div>
					<div class="account_actual-status_item" data-th="<?php esc_attr_e( 'Expiration / Renewal Date', 'rcp' ); ?>">
						<?php
	...
	<div class="account-payment_history" id="rcp-payment-history">
	<h4>History</h4>
		<header>
		<div class="payment-titles">
			<div class="payment-invoice"><?php _e( 'Invoice #', 'rcp' ); ?></div>
			<div class="payment_plan-type"><?php _e( 'Membership', 'rcp' ); ?></div>
			<div class="payment-method"><?php _e( 'Payment Status', 'rcp' ); ?></div>
			<div class="payment-amount"><?php _e( 'Amount', 'rcp' ); ?></div>
			<div class="payment-date"><?php _e( 'Date', 'rcp' ); ?></div>
			<div class="payment_invoice-load "><?php _e( 'Actions', 'rcp' ); ?></div>
		</div>
		</header>
		<main>
		<?php
		...
		if ( $payments ) : ?>
			<?php foreach ( $payments as $payment ) : ?>
				<div class="payment-info">
					<div class="payment-invoice " data-th="<?php esc_attr_e( 'Invoice #', 'rcp' ); ?>">
					<?php echo $payment->id; ?></div>
					<div class="payment_plan-type " data-th="<?php esc_attr_e( 'Membership', 'rcp' ); ?>">
					<?php echo esc_html( $payment->subscription ); ?></div>
					<div class="payment-method" data-th="<?php esc_attr_e( 'Amount', 'rcp' ); ?>">
					<?php  echo rcp_currency_filter( $payment->amount ); ?></div>
					<div class="payment-amount" data-th="<?php esc_attr_e( 'Payment Status', 'rcp' ); ?>">
					<?php echo rcp_get_payment_status_label( $payment ); ?></div>
					<div class="payment-date" data-th="<?php esc_attr_e( 'Date', 'rcp' ); ?>">
					<?php echo date_i18n( get_option( 'date_format' ), 
					strtotime( $payment->date, current_time( 'timestamp' ) ) ); ?>
				</div>
					<div class="payment_invoice-load" data-th="<?php esc_attr_e( 'Actions', 'rcp' ); ?>">
